...
class TestRESTAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        _ADMIN_HEADERS['Authorization'] = f'Bearer {self._admin_token}'
    ...
    def test_create_physical_device(self):
        dev, new_dev = self._create_physical_device(expected_code = 403, req_header = _HEADERS)

        dev, new_dev = self._create_physical_device(req_header = _ADMIN_HEADERS)
    ...
    def test_update_physical_device(self):
        ...
        r = requests.patch(url, headers=_ADMIN_HEADERS, data=payload)
        self.assertEqual(r.status_code, 200)
    ...
    def test_delete_physical_device(self):
        ...
        dev, new_dev = self._create_physical_device(expected_code = 201, req_header = _ADMIN_HEADERS)

        url=f'{_BASE}/physical/devices/{new_dev.uid}'
        # Delete using admin headers, should return 204 no content.
        r = requests.delete(url, headers=_ADMIN_HEADERS)
    ...
    def test_create_device_note(self):
        ...
        r = requests.post(url, headers=_ADMIN_HEADERS, data=note1.json())
    ...
    def test_get_device_notes(self):
        ...
        time.sleep(0.01)
        r = requests.post(url, headers=_ADMIN_HEADERS, data=note2.json())
        self.assertEqual(r.status_code, 201)

        time.sleep(0.01)
        r = requests.post(url, headers=_ADMIN_HEADERS, data=note3.json())
        self.assertEqual(r.status_code, 201)
    ...
    def test_create_logical_device(self):
        dev, new_dev = self._create_default_logical_device(expected_code = 403, req_header = _HEADERS)

        dev, new_dev = self._create_default_logical_device(expected_code = 201, req_header = _ADMIN_HEADERS)
    ...
    def test_update_logical_device(self):
        dev, new_dev = self._create_default_logical_device(req_header=_ADMIN_HEADERS)
    ...
    def test_insert_mapping(self):
        pdev, new_pdev = self._create_physical_device(req_header=_ADMIN_HEADERS)
        ldev, new_ldev = self._create_default_logical_device(req_header=_ADMIN_HEADERS)
        mapping = PhysicalToLogicalMapping(pd=new_pdev, ld=new_ldev, start_time=self.now())

        # This should work.
        url=f'{_BASE}/mappings/'
        payload = mapping.json()
        r = requests.post(url, headers=_ADMIN_HEADERS, data=payload)
        self.assertEqual(r.status_code, 201)
        ...
        requests.patch(f'{url}physical/end/{mapping.pd.uid}', headers=_ADMIN_HEADERS)
        mapping.start_time=self.now()
        payload = mapping.json()
        r = requests.post(url, headers=_ADMIN_HEADERS, data=payload)
        self.assertEqual(r.status_code, 201)
    ...
    def test_get_mapping_from_logical(self):
        ...
        # End the current mapping for the phyiscal device so the RESTAPI doesn't
        # return status 400.
        requests.patch(f'{url}logical/end/{mapping.ld.uid}', headers=_ADMIN_HEADERS)
    ...
    def test_get_latest_mapping_from_logical(self):
        ...
        # End the mapping to test that current returns None but latest returns the finished mapping.
        requests.patch(f'{url}logical/end/{new_ldev.uid}', headers=_ADMIN_HEADERS)
    ...
    def test_get_all_logical_device_mappings(self):
        ...
        pdev2.name = 'D2'
        pdev2, new_pdev2 = self._create_physical_device(dev=pdev2, req_header=_ADMIN_HEADERS)
        time.sleep(0.1)
        ...
        pdev3.name = 'D3'
        pdev3, new_pdev3 = self._create_physical_device(dev=pdev3, req_header=_ADMIN_HEADERS)
        time.sleep(0.1)
