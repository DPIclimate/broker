...
def run_check() -> str:
    ...
    FIXME: Add a check to see if the entire node has gone missing. For example during
    development node 325 went offline and is getting reported in both the missing nodes
    table and the missing sensors table.

    It should not be in the missing sensor table because it is an offline node so should
    only be reported there.

    The quality.devs variable can be used for this check, as the missing nodes test does.
    """
