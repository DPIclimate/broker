...
import asyncio, datetime, dateutil.parser, hashlib, json, logging, os, pathlib, re, requests, signal, time, uuid
...
def poll() -> None:
    logger.info(f"Start polling EagleIO nodes.")
    ...
    if _eagleio_token is not None:
        ...
        logger.error(f"Error polling EagleIO: No API token available.")
...
def process_sensor_node(node_name, node_sensors) -> None:
    ...
    logger.info(f"Message Hash: {hash}")
    ...
    logger.info(f"Sensor group {sg_id_str} last seen at {max_ts}.")
...
def get_nodes(_headers) -> None:
    ...
    logger.info(f"Requesting EagleIO nodes from {url}...")
    try:
        request = requests.get(url, headers=_headers, timeout=10)
    except Exception as e:
        request = None
        logger.error(f"Request timeout getting EagleIO nodes. \n{e}")

    if request is not None:
        if request.status_code == 200:
            logger.info(f"Got EagleIO nodes.")
            nodes = request.json()
            return nodes
        else:
            logger.error(f"Error getting EagleIO nodes: {request.status_code} {request.text}")
            return None
    else:
        return None
...
def main():
    ...
    logger.info('===============================================================')
    logger.info('                  STARTING Eagle.IO POLLER')
    logger.info('===============================================================')
